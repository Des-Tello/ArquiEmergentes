...
def sensor_data_get_all(company_api_key):
    db = get_db()
    cursor = db.cursor()
    query = "SELECT SENSOR_DATA.TIME, SENSOR_DATA.DATA, SENSOR_DATA.VALUE, COMPANY.ID, LOCATION.ID, SENSOR.ID FROM COMPANY, LOCATION, SENSOR, SENSOR_DATA WHERE SENSOR.SENSOR_API_KEY = SENSOR_DATA.SENSOR_API_KEY AND LOCATION.COMPANY_ID = COMPANY.ID AND SENSOR.LOCATION_ID = LOCATION.ID AND COMPANY.COMPANY_API_KEY = ?"
    result = cursor.execute(query, (company_api_key,))
    result = result.fetchall()
